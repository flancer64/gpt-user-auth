...
describe('Fl64_Gpt_User_Back_Store_RDb_Schema_User_Session', () => {
    ...
    it('should have the correct ENTITY name and primary key', async () => {
        assert.equal(rdbDto.getEntityName(), `${DEF.NAME}/fl64/gpt/user/session`, 'Entity name should match the expected path');
        assert.deepStrictEqual(rdbDto.getPrimaryKey(), [ATTR.SESSION_ID], 'Primary key should be set to SESSION_ID');
    });
});