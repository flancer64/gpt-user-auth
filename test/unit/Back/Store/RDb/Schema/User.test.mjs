...
describe('Fl64_Gpt_User_Back_Store_RDb_Schema_User', () => {
    ...
    it('should have the correct ENTITY name and primary key', async () => {
        assert.equal(rdbDto.getEntityName(), `${DEF.NAME}/fl64/gpt/user`, 'Entity name should match the expected path');
        assert.deepStrictEqual(rdbDto.getPrimaryKey(), [ATTR.USER_REF], 'Primary key should be set to USER_REF');
    });
});
